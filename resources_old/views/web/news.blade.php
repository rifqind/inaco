                ...
                <div class="row justify-content-center">
                    <div class="col-md-8">
                        ...
                        @else
                                ...
                                @elseif ($code == 'ar')
                                    <h2 class="article-title"><span>المقالات الأحدث</span></h2>
                                @elseif ($code == 'vi')
                                ...
                                @if ($popularNews->isNotEmpty())
                                    <div id="popular-article">
                                        <h2 class="text-center">
                                            @if ($code == 'id')
                                                <div class="text-center">Artikel Populer</div>
                                            @elseif ($code == 'ar')
                                                <div class="text-center">المقالات الشائعة</div>
                                            @elseif ($code == 'vi')
                                                <div class="text-center">Bài viết phổ biến</div>
                                            @else
                                                <div class="text-center">Popular Articles</div>
                                            @endif
                                        </h2>
                                        @foreach ($popularNews as $value)
                                            <div class="list-article d-flex">
                                                <div class="image-popular"><img
                                                        src="{{ asset('data/news') . '/' . $value->news_image }}"></div>
                        ...
                        <div class="col-sm-6 text-center text-sm-start">
                            @if ($code == 'id')
                                <div class="show-perpage text-sm">Menampilkan 1-{{ $news->count() }} dari {{ $news->total() }}
                                    berita</div>
                            @elseif($code == 'ar')
                                <div class="show-perpage text-sm">عرض 1-{{ $news->count() }} من {{ $news->total() }} خبر</div>
                            @elseif($code == 'vi')
                                <div class="show-perpage text-sm">Hiển thị 1-{{ $news->count() }} trong số {{ $news->total() }}
                                    tin tức</div>
                            @else
                                <div class="show-perpage text-sm">Showing 1-{{ $news->count() }} of {{ $news->total() }} news
                                </div>
                            @endif
                        </div>
                        @if ($news->lastPage() != 1)
